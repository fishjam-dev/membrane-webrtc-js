...
test("Invalid track metadata doesn't get pushed to the server when adding track", async ({ page: firstPage, context }) => {
  const secondPage = await context.newPage();
  await firstPage.goto("/");
  await secondPage.goto("/");
...
test("Invalid track metadata doesn't get pushed to the server when updating track metadata", async ({ page: firstPage, context }) => {
  const secondPage = await context.newPage();
  await firstPage.goto("/");
  await secondPage.goto("/");
  ...
  const firstClientId = await joinRoom(firstPage, roomId, { goodStuff: "ye" });
  const secondClientId = await joinRoom(secondPage, roomId, { goodStuff: "ye" });
  await secondPage.getByText("Add a heart").click();
