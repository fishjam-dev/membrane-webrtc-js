...
export interface TrackContext {
  ...
  /**
   * Track id. It is generated by RTC engine and takes form of `endpoint_id:<random_uuidv4>`.
...
export interface Callbacks {
   ...
   *
   * @param {string} origin - id of an endpoint that owns track
...
export class MembraneWebRTC {
  ...
  // Looks like it's a collection of local tracks
  private localTracks: Map<string, TrackContext> = new Map();
  private midToTrackId: Map<string, string> = new Map();

  // TrackId -> TrackEncoding[]
  ...
  private handleMediaEvent = (deserializedMediaEvent: MediaEvent) => {
    ...
    switch (deserializedMediaEvent.type) {
      ...
      case "tracksAdded":
        ...
        for (let [trackId, metadata] of data.trackIdToMetadata) {
          if (this.trackIdToTrack.has(trackId)) {
            console.error("Received duplicated trackId: " + trackId);
            continue;
          }
      ...
      case "tracksRemoved":
        ...
        for (let trackId of trackIds) {
          ...
          if(!trackContext) {
            console.error("Received `tracksRemoved` media event refering to the track that doesn't exist");
            continue;
          }

          this.callbacks.onTrackRemoved?.(trackContext);
          this.eraseTrack(trackId);
        }
        break;
      ...
      case "encodingSwitched":
        this.callbacks.onTrackEncodingChanged?.(
          // FIXME: this event doesn't need an origin
          deserializedMediaEvent.data.origin,
      ...
      case "vadNotification": {
        const trackId = deserializedMediaEvent.data.trackId;
        const origin = this.trackIdToTrack.get(trackId)!.origin;
   ...
   * @param {object} trackMetadata - Any information about this track that other peers will
   * receive in {@link onTrackAdded}. E.g. this can contain the source of the track - wheather it's
  ...
  public addTrack(
    ...
    this.localTracks.set(trackId, trackContext);
  ...
  public async replaceTrack(
    ...
    const trackContext = this.localTracks.get(trackId)!;
    ...
    if (sender) {
      return sender
        ...
        .then(() => {
          const trackMetadata =
            newTrackMetadata || this.localTracks.get(trackId)!.metadata;
  ...
  public enableTrackEncoding(trackId: string, encoding: TrackEncoding) {
    let track = this.localTracks.get(trackId)?.track;
  ...
  private getMidToTrackId = () => {
    ...
    this.connection.getTransceivers().forEach((transceiver) => {
      ...
      if (localTrackId && mid) {
        const trackContext = Array.from(this.localTracks.values()).find(
  ...
  public cleanUp = () => {
    ...
    for (let trackCtx of this.localTracks.values()) {
      trackCtx.track?.stop();
    }
  ...
  private getTrackIdToMetadata = () => {
    ...
    Array.from(this.localTracks.values()).forEach(
      (trackContext: TrackContext) => {
        trackIdToMetadata[trackContext.trackId] = trackContext.metadata;
      }
    );
  ...
  private onOfferData = async (offerData: Map<string, number>) => {
    if (!this.connection) {
      ...
      Array.from(this.localTracks.values()).forEach((trackContext) =>
        this.addTrackToConnection(trackContext)
      );
  ...
  private onTrack = () => {
    return (event: RTCTrackEvent) => {
      ...
      const {origin, metadata} = this.trackIdToTrack.get(trackId)!;
  ...
  private eraseTrack = (trackId: string) => {
