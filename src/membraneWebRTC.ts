...
export interface Endpoint {
  ...
  type: string;
  /**
   * Any information that was provided in {@link MembraneWebRTC.connect}.
   */
  metadata: any;
  /**
...
export interface TrackContext
  extends TrackContextFields,
    TypedEmitter<Required<TrackContextEvents>> {}
  ...
  implements TrackContext
{
  ...
  constructor(endpoint: Endpoint, trackId: string, metadata: any) {
    super();
...
/**
 * Events emitted by the {@link MembraneWebRTC} instance.
 */
export interface MembraneWebRTCEvents {
  /**
   * Emitted each time MembraneWebRTC need to send some data to the server.
   */
...
>) {
  ...
  /**
   * Tries to connect to the RTC Engine. If user is succesfully connected then {@link MembraneWebRTCEvents.connected}
   * will be emitted.
   *
   * @param metadata - Any information that other endpoints will receive in {@link MembraneWebRTCEvents.endpointAdded}
      ...
      case "connected":
        ...
        otherEndpoints.forEach((endpoint) => {
          Array.from(endpoint.trackIdToMetadata.entries()).forEach(
            ([trackId, metadata]) => {
              const ctx = new TrackContextImpl(endpoint, trackId, metadata);
  ...
  private handleMediaEvent = (deserializedMediaEvent: MediaEvent) => {
    ...
    switch (deserializedMediaEvent.type) {
      ...
      case "endpointRemoved":
        ...
        Array.from(endpoint.trackIdToMetadata.keys()).forEach((trackId) => {
      ...
      case "trackUpdated": {
        ...
        endpoint.trackIdToMetadata.set(trackId, trackMetadata);
        const trackContext = this.trackIdToTrack.get(trackId)!;
   ...
   * receive in {@link MembraneWebRTCEvents.endpointAdded}. E.g. this can source of the track - whether it's
  ...
  public addTrack(
    ...
    this.localEndpoint.trackIdToMetadata.set(trackId, trackMetadata);
  ...
  public removeTrack(trackId: string) {
    ...
    this.localEndpoint.trackIdToMetadata.delete(trackId);
  }
   ...
   * that endpoint was removed in {@link MembraneWebRTCEvents.endpointRemoved},
   */
  public leave = () => {
  ...
  private getTrackIdToMetadata = () => {
    ...
    Array.from(this.localEndpoint.trackIdToMetadata.entries()).forEach(
      ([trackId, metadata]) => {
  ...
  private checkIfTrackBelongToEndpoint = (
    ...
    Array.from(endpoint.trackIdToMetadata.keys()).some((track) =>
  ...
  private eraseEndpoint = (endpoint: Endpoint): void => {
    const tracksId = Array.from(endpoint.trackIdToMetadata.keys());
  ...
  private eraseTrack = (trackId: string, endpointId: string) => {
    ...
    this.idToEndpoint.get(endpointId)!.trackIdToMetadata.delete(trackId);
