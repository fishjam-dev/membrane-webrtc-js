...
export interface Endpoint {
  ...
  metadata: any;
  /**
   * Mapping between track's id (generated by rtc engine) and its metadata and simulcastConfig. Both track metadata
   * and simulcastConfig can be set using {@link WebRTCEndpoint.addTrack}. Track id is generated by RTC engine.
   */
  tracks: Map<string, [any, SimulcastConfig]>;
}
...
>) {
      ...
      case "connected":
        ...
        otherEndpoints.forEach((endpoint) => {
          ...
          Array.from(endpoint.tracks.entries()).forEach(
            ([trackId, [metadata, simulcastConfig]]) => {
  ...
  private handleMediaEvent = (deserializedMediaEvent: MediaEvent) => {
    ...
    switch (deserializedMediaEvent.type) {
      ...
      case "tracksAdded":
        ...
        data.tracks = new Map<string, any>(
      ...
      case "trackUpdated": {
        ...
        const simulcastConfig: SimulcastConfig = endpoint.tracks.get(trackId)!![1];
        endpoint.tracks.set(trackId, [trackMetadata, simulcastConfig]);
        const trackContext = this.trackIdToTrack.get(trackId)!;
  ...
  public addTrack(
    ...
    const trackEntry: [any, SimulcastConfig] = [trackMetadata, simulcastConfig]

    this.localEndpoint.tracks.set(trackId, trackEntry);
  ...
  public updateTrackMetadata = (trackId: string, trackMetadata: any): void => {
    ...
    const simulcastConfig = this.localEndpoint.tracks.get(trackId)!![1];

    this.localEndpoint.tracks.set(trackId, [trackMetadata, simulcastConfig]);
    const mediaEvent = generateMediaEvent("updateTrackMetadata", {
      trackId,
      trackMetadata,
    });
