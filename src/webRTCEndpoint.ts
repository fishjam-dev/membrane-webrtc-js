 ...
 */
export interface Endpoint<PeerMetadata, TrackMetadata> {
  /**
   ...
   */
  tracks: Map<string, TrackContextImpl<PeerMetadata, TrackMetadata>>;
}
 ...
 */
interface TrackContextFields<PeerMetadata, TrackMetadata> {
   ...
   */
  readonly endpoint: Endpoint<PeerMetadata, TrackMetadata>;
...
export interface TrackContextEvents<PeerMetadata, TrackMetadata> {
  /**
   ...
   */
  encodingChanged: (context: TrackContext<PeerMetadata, TrackMetadata>) => void;

  /**
   * Emitted every time an update about voice activity is received from the server.
   */
  voiceActivityChanged: (context: TrackContext<PeerMetadata, TrackMetadata>) => void;
}

export interface TrackContext<PeerMetadata, TrackMetadata>
  extends TrackContextFields<PeerMetadata, TrackMetadata>,
    TypedEmitter<Required<TrackContextEvents<PeerMetadata, TrackMetadata>>> {}

type TrackNegotiationStatus = "awaiting" | "offered" | "done";

class TrackContextImpl<PeerMetadata, ParsedMetadata>
  extends (EventEmitter as {
    new <PeerMetadata, ParsedMetadata>(): TypedEmitter<Required<TrackContextEvents<PeerMetadata, ParsedMetadata>>>;
  })<PeerMetadata, ParsedMetadata>
  implements TrackContext<PeerMetadata, ParsedMetadata>
{
  endpoint: Endpoint<PeerMetadata, ParsedMetadata>;
  trackId: string;
 ...
 */
export interface WebRTCEndpointEvents<PeerMetadata, TrackMetadata> {
  /**
   ...
   */
  connected: (endpointId: string, otherEndpoints: Endpoint<PeerMetadata, TrackMetadata>[]) => void;
   ...
   */
  endpointAdded: (endpoint: Endpoint<PeerMetadata, TrackMetadata>) => void;

  /**
   * Emitted each time endpoint is removed, emitted only for other endpoints.
   */
  endpointRemoved: (endpoint: Endpoint<PeerMetadata, TrackMetadata>) => void;

  /**
   * Emitted each time endpoint has its metadata updated.
   */
  endpointUpdated: (endpoint: Endpoint<PeerMetadata, TrackMetadata>) => void;
  ...
  public addTrack(
    ...
    try {
      ...
      this.pushCommand({
        ...
        track,
        stream,
        trackMetadata: parsedTrackMetadata,
        simulcastConfig,
        maxBandwidth,
        resolutionNotifier,
      });
