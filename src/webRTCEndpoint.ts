 ...
 */
export interface Endpoint<TrackMetadata> {
  /**
   ...
   */
  tracks: Map<string, TrackContextImpl<TrackMetadata>>;
}
 ...
 */
interface TrackContextFields<TrackMetadata> {
   ...
   */
  readonly endpoint: Endpoint<TrackMetadata>;
...
export interface TrackContextEvents<TrackMetadata> {
  /**
   ...
   */
  encodingChanged: (context: TrackContext<TrackMetadata>) => void;

  /**
   * Emitted every time an update about voice activity is received from the server.
   */
  voiceActivityChanged: (context: TrackContext<TrackMetadata>) => void;
}

export interface TrackContext<TrackMetadata> extends TrackContextFields<TrackMetadata>, TypedEmitter<Required<TrackContextEvents<TrackMetadata>>> {}

type TrackNegotiationStatus = "awaiting" | "offered" | "done";

class TrackContextImpl<ParsedMetadata>
  extends (EventEmitter as { new<ParsedMetadata>(): TypedEmitter<Required<TrackContextEvents<ParsedMetadata>>>} )<ParsedMetadata>
  implements TrackContext<ParsedMetadata>
{
  endpoint: Endpoint<ParsedMetadata>;
  trackId: string;
  ...
  constructor(endpoint: Endpoint<ParsedMetadata>, trackId: string, metadata: any, simulcastConfig: SimulcastConfig, metadataValidator: MetadataValidator<ParsedMetadata>) {
    super();
 ...
 */
export interface WebRTCEndpointEvents<TrackMetadata> {
  /**
   ...
   */
  connected: (endpointId: string, otherEndpoints: Endpoint<TrackMetadata>[]) => void;
   ...
   */
  trackReady: (ctx: TrackContext<TrackMetadata>) => void;
   ...
   */
  trackAdded: (ctx: TrackContext<TrackMetadata>) => void;
   ...
   */
  trackRemoved: (ctx: TrackContext<TrackMetadata>) => void;

  /**
   * Emitted each time endpoint has its track metadata updated.
   */
  trackUpdated: (ctx: TrackContext<TrackMetadata>) => void;

  /**
   * Emitted each time new endpoint is added to the room.
   */
  endpointAdded: (endpoint: Endpoint<TrackMetadata>) => void;

  /**
   * Emitted each time endpoint is removed, emitted only for other endpoints.
   */
  endpointRemoved: (endpoint: Endpoint<TrackMetadata>) => void;

  /**
   * Emitted each time endpoint has its metadata updated.
   */
  endpointUpdated: (endpoint: Endpoint<TrackMetadata>) => void;
 ...
 */
export class WebRTCEndpoint<trackMetadata = any> extends (EventEmitter as { new<trackMetadata>(): TypedEmitter<Required<WebRTCEndpointEvents<trackMetadata>>> })<trackMetadata> {
  private trackIdToTrack: Map<string, TrackContextImpl<trackMetadata>> = new Map();
  private connection?: RTCPeerConnection;
  private idToEndpoint: Map<string, Endpoint<trackMetadata>> = new Map();
  private localEndpoint: Endpoint<trackMetadata> = {
    id: "",
    type: "webrtc",
    metadata: {} as trackMetadata,
    tracks: new Map(),
  };
  private localTrackIdToTrack: Map<string, TrackContextImpl<trackMetadata>> = new Map();
  ...
  constructor(validators?: { trackMetadataValidator: MetadataValidator<trackMetadata>}) {
    super();
  ...
  public receiveMediaEvent = (mediaEvent: SerializedMediaEvent) => {
    ...
    switch (deserializedMediaEvent.type) {
      case "connected": {
        ...
        const otherEndpoints: Endpoint<trackMetadata>[] = endpoints.map((endpoint) => {
   ...
   * }
   */
  public getRemoteTracks(): Record<string, TrackContext<trackMetadata>> {
  ...
  private addTrackCommandHandler(addTrackCommand: AddTrackCommand) {
    ...
    const trackContext = new TrackContextImpl(this.localEndpoint, trackId, trackMetadata, simulcastConfig, this.trackMetadataValidator);
  ...
  private checkIfTrackBelongToEndpoint = (trackId: string, endpoint: Endpoint<trackMetadata>) =>
  ...
  private addEndpoint = (endpoint: Endpoint<trackMetadata>): void => {
  ...
  private eraseEndpoint = (endpoint: Endpoint<trackMetadata>): void => {
  ...
  private mapMediaEventTracksToTrackContextImpl = (
    tracks: [string, any][],
    endpoint: Endpoint<trackMetadata>,
  ): Map<string, TrackContextImpl<trackMetadata>> => {
    const mappedTracks: Array<[string, TrackContextImpl<trackMetadata>]> = Array.from(tracks).map(([trackId, track]) => [
      trackId,
